...
action.getPolicyInfo = (req, res) => {
  ...
  action.fundSwitchSelectGoal = (req, res) =>{
    ...
    .then(body =>{
        console.log('JJJJ', JSON.stringify(body.result));
        let data = body;
            ...
            messages: [
              {
                "type": 0,
                "platform": "facebook",
                "speech": "Thank you!! Based on your selection, the below auto switch request will be processed. </br></br> <b>Before Fund Switch</b></br> Total Value(Equity) :" + data.BeforeCashVal + "</br> UnitBalance(Equity) :" + data.BeforeOutstanding +  "</br> Current  %(Equity) :" + data.BeforePercentSplit  + "</br> Total Value(Bond) :" + data.BeforeMMCashVal +  "</br> UnitBalance (Bond) :" + data.BeforeMMOutstanding + "</br> Current  %(Bond) :" + data.BeforeMMPercentSplit 
              },
              {
                  "type": 4,
                  "platform": "facebook",
